      ...
      <AnimatePresence>
        ...
        <div className="flex-1 overflow-y-auto p-6 space-y-4">
          {items.length === 0 ? (
            ...
            items.map(({ product, size, quantity }) => {
              ...
              return (
                <div key={`${product.id}-${size ?? 'na'}`} className="flex items-center gap-4">
                  ...
                  <button onClick={() => removeItem(product.id, size)} className="text-xs text-rose-600 hover:underline">Quitar</button>
                </div>
              );
            })
          )}
        ...
        {items.length > 0 && (
          <footer className="p-6 border-t border-gray-200 space-y-4">
            ...
            <button className="w-full bg-blue-600 text-white font-medium py-3 rounded-lg shadow hover:bg-blue-700 active:scale-95 transition-transform" onClick={() => {
              toggleCart();
