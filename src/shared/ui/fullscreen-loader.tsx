...
import styles from '@/styles/shared/fullscreen-loader.module.css';
...
export default function FullscreenLoader({ open, label = 'LOADING', ariaLabel = 'loading' }: FullscreenLoaderProps) {
  if (!open) return null;
  const letters = Array.from(label);
  return (
    <div className={`${styles.overlay} fl-overlay`} aria-label={ariaLabel} role="status" aria-live="polite" tabIndex={-1}>
      <div className={styles.shadow}>
        ...
        <div className="loading loading02" aria-hidden>
          {letters.map((ch, idx) => (
