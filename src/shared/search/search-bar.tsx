...
export default function SearchBar() {
  ...
  const handleKeyDown = (event: React.KeyboardEvent<HTMLInputElement>) => {
    if (event.key === 'ArrowDown') {
      ...
      setSelectedItemIndex((i) => (i < dataFiltered.length - 1 ? i + 1 : i));
      ...
      event.preventDefault();
      const target =
        selectedItemIndex !== -1 ? dataFiltered[selectedItemIndex] : data.find((p) => p.name.toLowerCase() === searchQuery.toLowerCase());
      if (target) router.push(`/product/${target.documentId}`);
      else setError('no se encontró un producto con ese nombre.');
      setInputFocus(false);
  ...
  return (
    <div className='relative flex w-full text-gray-600'>
      ...
      <div className={`absolute bg-white/70 border border-black/10 backdrop-blur-2xl w-full top-11 overflow-hidden rounded-sm z-50 transition-all duration-300 ease-in-out ${inputFocus ? 'max-h-[340px] opacity-100' : 'max-h-0 opacity-0'}`}>
        <div id='smScroll' ref={scrollContainerRef} className='flex flex-col max-h-[300px] p-1 overflow-y-auto'>
          {error ? (
            ...
            <div className='p-3 text-center'>No se encontró el producto</div>
          )}
        </div>
      </div>
    </div>
